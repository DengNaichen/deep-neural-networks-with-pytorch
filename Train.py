...
def train_free_energy_two_inputs(net, x, inputs, rho, lambd, scheduler, optimizer, matrix, epochs, diagram=True):
    """
...
def train_free_energy_one_inputs(net, x, rho, lambd, scheduler, optimizer, matrix, epochs, diagram=True):
    """
    :param net: NN with random parameters
    :param x: an array, from 0 to 2pi
    :param inputs: inputs is an matrix with dimension (len(x), 2), sin(x) and cos(x)
    ...
    for i in range(epochs):
        yhat = net(x)  # the different from previous one
